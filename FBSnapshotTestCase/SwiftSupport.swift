...
*/
  ...
  public extension FBSnapshotTestCase {
    public func FBSnapshotVerifyView(_ view: UIView,
                                     ...
                                     tolerance: CGFloat = 0,
                                     colorTolerance: CGFloat = 0,
                                     file: StaticString = #file,
                                     line: UInt = #line) {
      
      FBSnapshotVerifyViewOrLayer(view,
    ...
    public func FBSnapshotVerifyLayer(_ layer: CALayer,
      ...
      FBSnapshotVerifyViewOrLayer(layer,
                                  identifier: identifier,
                                  suffixes: suffixes,
                                  tolerance: tolerance,
                                  colorTolerance: colorTolerance,
      ...
      if let envReferenceImageDirectory = envReferenceImageDirectory {
        for suffix in suffixes {
          ...
          if viewOrLayer.isKind(of: UIView.self) {
            do {
              try compareSnapshot(of: viewOrLayer as! UIView,
            ...
            do {
              try compareSnapshot(of: viewOrLayer as! CALayer,
                                  referenceImagesDirectory: referenceImagesDirectory,
                                  identifier: identifier,
                                  tolerance: tolerance,
                                  colorTolerance: colorTolerance)
          ...
          assert(comparisonSuccess, message: "Snapshot comparison failed: \(error.map(String.init(describing:)) ?? "unknown error")", file: file, line: line)
        }
      } else {
...
public extension FBSnapshotTestCase {
  public func FBSnapshotVerifyView(view: UIView,
                                  identifier: String = "",
                                  suffixes: NSOrderedSet = FBSnapshotTestCaseDefaultSuffixes(),
                                  tolerance: CGFloat = 0,
                                  colorTolerance: CGFloat = 0,
  ...
  public func FBSnapshotVerifyLayer(layer: CALayer,
                                    identifier: String = "",
                                    suffixes: NSOrderedSet = FBSnapshotTestCaseDefaultSuffixes(),
                                    tolerance: CGFloat = 0,
                                    colorTolerance: CGFloat = 0,
  ...
  private func FBSnapshotVerifyViewOrLayer(viewOrLayer: AnyObject,
                                           identifier: String,
                                           suffixes: NSOrderedSet,
                                           tolerance: CGFloat,
                                           colorTolerance: CGFloat,
    ...
    if let envReferenceImageDirectory = envReferenceImageDirectory {
      for suffix in suffixes {
        ...
        if viewOrLayer.isKindOfClass(UIView) {
          do {
            try compareSnapshotOfView(viewOrLayer as! UIView,
                                      referenceImagesDirectory: referenceImagesDirectory,
                                      identifier: identifier,
                                      tolerance: tolerance,
                                      colorTolerance: colorTolerance)
          ...
          do {
            try compareSnapshotOfLayer(viewOrLayer as! CALayer,
                                       referenceImagesDirectory: referenceImagesDirectory,
                                       identifier: identifier,
                                       tolerance: tolerance,
                                       colorTolerance: colorTolerance)
